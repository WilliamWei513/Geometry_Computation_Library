...
def sort_list(keys_tree, values_tree=None, reverse=False):
    ...
    for path in keys_tree.Paths:
        ...
        if values_tree is not None:
            try:
                if use_values_broadcast:
                    vals = values_single_items
                else:
                    vals = list(values_tree.Branch(path)) if path in values_tree.Paths else []
            except:
                vals = []
...
exploded = explode_curves(curves)
curves_clockwise = join_curves(exploded, tolerance=1e-6, rg.Plane.WorldXY) 